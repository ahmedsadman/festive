...
class PaginatedResponse:
    """Show paginated response in a proper format. Not an actual schema,
    because it will be only used to dump data."""
...
class ParticipantSchema(BaseSchema):
    ...
    tshirt_size = fields.Str(
        required=True, validate=validate.OneOf(["s", "m", "l", "xl", "xxl"])
    )
    events = fields.Nested("EventSchema", only=("id", "name"), many=True)
    event_id = fields.Int()
    teams = fields.Nested(
        "TeamSchema", only=("id", "name", "event_id"), many=True
    )
...
class TeamSchema(BaseSchema):
    ...
    payment_status = fields.Str(
        validate=validate.OneOf(["pending", "waiting", "ok"])
    )
...
class EventRegistration(BaseSchema):
    participants = fields.Nested(
        ParticipantSchema(
            only=("name", "email", "institute", "tshirt_size", "contact_no")
        ),
        many=True,
        required=True,
    )
    team_name = fields.Str(required=True, validate=validate.Length(max=50))
    participation_level = fields.Str(
        missing=None,
